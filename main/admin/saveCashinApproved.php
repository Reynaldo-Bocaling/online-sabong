...
if(!isset($_SESSION['companyID'])){
	...
	if(isset($_POST['cashinID'])){
		...
		if($query->num_rows > 0){
			...
			$update = $mysqli->query("UPDATE `tblusertransactions` SET statusID = '0' WHERE id = '".$cashinID."' AND transactionID = '1'  ");
			
			if($update){
				$logAction = "Approved the Cash IN transaction from teller" . $username . " with transaction Code: ".$transactionCode;
