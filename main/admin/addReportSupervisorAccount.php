...
if(!isset($_SESSION['companyID'])){
	...
	if(isset($_POST['username'])){
		...
		if($query->num_rows > 0){
			...
			$insert = $mysqli->query("INSERT INTO `tblusers`(`id`, `username`, `password`, `cname`, `roleID`, `isActive`) VALUES ('', '".$username."', '".md5($username)."', '".strtoupper($systemname)."', '12', '1')");
			
			if($insert){
				$logAction = "Added report supervisor account, username: ".$username;
