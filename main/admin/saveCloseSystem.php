...
if(!isset($_SESSION['companyID'])){
	...
	if(isset($_POST['eventID'])){
		...
		$qcheck = $mysqli->query("SELECT `id` FROM `tblevents` WHERE `userAccessStatus` = '0' ORDER BY id DESC LIMIT 1; ");
		if($qcheck->num_rows > 0){
			...
			if($qcheck->num_rows > 0 ){
				...
				if($isBetting == 5 OR $isBetting == 6){
					$u = $mysqli->query("UPDATE `tblevents` SET `userAccessStatus` = '1' WHERE id = '".$eventID."'");
					if($u){
						$logAction = "Closed the system to user access";
