...
if(!isset($_SESSION['companyID'])){
	...
	if(isset($_POST['bettingClose'])){
		...
		if($qcheck->num_rows > 0){
			...
			if($rcheck['isBetting'] == 4){
				$u = $mysqli->query("UPDATE `tblfights` SET isBetting = '2' ORDER BY id DESC LIMIT 1 ");
				if($u){
					$query = $mysqli->query("SELECT `fightCode` FROM `tblfights` ORDER BY id DESC LIMIT 1");
					$row = $query->fetch_assoc();
					$fightCode = $row['fightCode'];
					$logAction =  $fightCode.": Closed fight bettings";
