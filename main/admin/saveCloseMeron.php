	...
	if(isset($_POST['closeMeron'])){
		...
		if($qcheck->num_rows > 0){
			...
			if($rcheck['isBetting'] == 1 OR $rcheck['isBetting'] == 4){ // check if Current Fight Status is OPEN or LAST CALL
				$u = $mysqli->query("UPDATE `tblfights` SET `closeMeron` = '1' WHERE id = '".$fightID."' ");
				if($u){
					$logAction = $fightCode.": Bettings for MERON was temporarily Close.";
