...
<div id="wrapper" class="fixed top-0 left-0 w-screen h-screen overflow-y-auto">
					...
					<table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
						<thead>
							<tr>
								...
								<th class="text-sm" style="text-align:center;">Amount/Points</th>
								<th class="text-sm" style="text-align:center;">Mobile Number</th>
						...
						<tbody>
						<?php
							$qtrans = $mysqli->query("SELECT a.`id`, a.`accountID`, a.`transDate`, a.`transCode`, a.`transAmount`, a.`isProcess`, b.`mobileNumber` FROM `tblnewbalance` a LEFT JOIN `tblaccounts` b ON a.accountID = b.id WHERE a.`transID` = '1' AND a.`isProcess` = '0' ORDER BY a.id DESC");
							if($qtrans->num_rows > 0){
								...
								while($rtrans = $qtrans->fetch_assoc()){
									...
									<tr  style="text-align:center;">
										...
										<td><button class="text-xs text-white font-medium px-4 py-2 rounded-full bg-blue-500 btnDepositConfirm" value = "'.$rtrans['transCode'].'" data-accountID ="'.$rtrans['accountID'].'"  data-id="'.$rtrans['id'].'">ENTER PIN</button</td>
										<td class="text-sm">'.$rtrans['transCode'].'</td>
										<td class="text-sm" style="text-align:right;">'.number_format($rtrans['transAmount'],2).'</td>
										<td class="text-sm">'.$rtrans['mobileNumber'].'</td>
										<td class="text-sm">'.$isProcess.'</td>
										<td class="text-sm">'.$rtrans['transDate'].'</td>
									</tr>';
									$count++;
								}
							}
  ...
  <script src="design/vendor/datatables/dataTables.bootstrap4.min.js"></script>
	...
	<script>
		function myFunction() {
		   pinVal = prompt("Please Enter Deposit PIN", "");
			if (pinVal != null) {
				transCodeVal = $("#hiddenTransCode").val();
				accountIDVal = $("#hiddenAccountID").val();
				idVal = $("#hiddenNewBalanceID").val();
				$.post("admin/confirmAccountBalanceDeposit.php", {transCode:transCodeVal, accountID:accountIDVal, transPin:pinVal, transID:idVal}, function(res){
					if(res == 1){
						swal({
		...
		$(document).ready(function(){
			...
			$(".btnDepositConfirm").click(function(){
