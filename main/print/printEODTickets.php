...
if(!isset($_SESSION['companyID'])){
	...
	if(isset($_POST['generate_tellersummaryreport'])){
		$result = 0;
		$eventID = mysqli_real_escape_string($mysqli, $_POST['hiddenEODTicketEventID']);
...
if($query2->num_rows > 0){
	while($row2 = $query2->fetch_assoc()){
		...
		if($row2['isBetting'] == 6 AND $row2['isWinner'] == 3 AND $row2['isReturned'] == 0){ //getting the total draw unpaid
		...
		if($row2['isBetting'] == 6 AND ($row2['betType'] == $row2['isWinner']) AND $row2['isClaim'] == 0){
							...
							<body onload = "window.print()" style="padding:3px;">
								<div class="ticket">
									<p style="margin-top:-10px; margin-left:10px;">									
										...
										<span style="font-size:10px; font-weight:bold;">TICKETING BETS</span><br/>
										<br/>
										...
										<span style="font-size:10px;">Total APP deposit: '.number_format($mobileDeposit,2).'</span><br/>
										<br/>
										...
										<span style="font-size:10px;">Total APP withdraw: '.number_format($mobileWithdraw,2).'</span><br/>
										<br/>
